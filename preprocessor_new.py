...
if __name__ == '__main__':
    ...
    def index_merger(path):
        ...
        temp = os.listdir('.' + os.altsep + 'temp_' + str(args[2]))
        temp_path_list = []
        for item in temp:
            file_path = os.path.join('.' + os.altsep + 'temp_' + str(args[2]), item)
        ...
        if len(args) < 5:
            ...
            def two_way_merge(f1, f2, limit, count, temp_path_list):
                ...
                output = open('.' + os.altsep + 'temp_' + str(args[2]) + os.altsep + 'output_%i.csv' % count, 'w')
    ...
    def position_temp_writer(document_ID, term_frequency_counter, position_counter):
        with open('.' + os.altsep + 'temp_' + str(args[2]) + os.altsep + '%s.csv' % document_ID, 'w', newline='') as csv_file:
        ...
        if len(args) < 5:
            ...
            with open('.' + os.altsep + 'temp.csv', 'r', newline='') as csv_file:
                ...
                for data in csv_reader:
                    ...
                    # count all triplets in list as document frequency
                for key in term_info.keys():
                    if document_frequency_counter.get(key) >= 3:
                        index[key] = [document_frequency_counter.get(key), term_info.get(key)]
                    # index format: key: [document_frequency, [(document_ID, term_frequency), (document_ID, term_frequency)]]
                for key in sorted(index.keys()):
                    write_list = [key, index.get(key)[0]]
    ...
    def normalization(line, bag, document_ID):
        ...
        #         token = re.sub('-', '', token)
        #         bag.append((token.strip(), document_ID))  # store F-16 into F16
        #
    ...
    if not os.path.exists('.' + os.altsep + 'temp_' + str(args[2])):
        os.makedirs('.' + os.altsep + 'temp_' + str(args[2]))
    if not os.path.exists(args[3]):
        os.makedirs(args[3])

    if len(args) < 5:
        if args[2] == 'stem':
            stem_index_path = os.path.join(args[3], 'stem_index_no_limit.csv')
            ...
            position_index_path = os.path.join(args[3], 'single_term_positional_index_no_limit.csv')
            ...
            phrase_index_path = os.path.join(args[3], 'phrase_index_no_limit.csv')
            ...
            single_index_path = os.path.join(args[3], 'single_term_index_no_limit.csv')
        ...
        if args[2] == 'stem':
            stem_name = 'stem_index_' + str(args[4]) + '.csv'
            stem_index_path = os.path.join(args[3], stem_name)
            stem_index_tokenizer(open(collection_path))
            index_merger(stem_index_path)
            ...
            new_name = os.path.join('.' + os.altsep + 'temp', stem_name)
            ...
            position_name = 'single_term_positional_index_' + str(args[4]) + '.csv'
            position_index_path = os.path.join(args[3], position_name)
            position_index_tokenizer(open(collection_path))
            index_merger(position_index_path)
            temp = os.listdir('.' + os.altsep + 'temp')
            temp_path_list = []
            for item in temp:
                ...
                temp_path_list.append(file_path)
            old_name = temp_path_list[0]
            new_name = os.path.join('.' + os.altsep + 'temp', position_name)
            os.rename(old_name, new_name)
            shutil.move(new_name, position_index_path)
            open('.' + os.altsep + 'temp.csv', 'w').truncate()
            gc.collect()
        elif args[2] == 'phrase':
            phrase_name = 'phrase_index_' + str(args[4]) + '.csv'
            phrase_index_path = os.path.join(args[3], phrase_name)
            phrase_index_tokenizer(open(collection_path))
            phrase_index_merger()
            temp = os.listdir('.' + os.altsep + 'temp')
            temp_path_list = []
            for item in temp:
                ...
                temp_path_list.append(file_path)
            old_name = temp_path_list[0]
            new_name = os.path.join('.' + os.altsep + 'temp', phrase_name)
            os.rename(old_name, new_name)
            shutil.move(new_name, phrase_index_path)
            open('.' + os.altsep + 'temp.csv', 'w').truncate()
            gc.collect()
        elif args[2] == 'single':
            single_name = 'single_term_index_' + str(args[4]) + '.csv'
            single_index_path = os.path.join(args[3], single_name)
            single_index_tokenizer(open(collection_path))
            index_merger(single_index_path)
            temp = os.listdir('.' + os.altsep + 'temp')
            temp_path_list = []
            for item in temp:
                ...
                temp_path_list.append(file_path)
            old_name = temp_path_list[0]
            new_name = os.path.join('.' + os.altsep + 'temp', single_name)
            os.rename(old_name, new_name)
